...
initVar() {
	...
	#Current Personalized Installation 01234
	...
	#Choose the personalized installation method
	selectCustomInstallType=

	# v2ray-core , path of xray-core configuration files
	configPath=
...
installTools() {
	...
	if [[ ! -d "$HOME/.acme.sh" ]] || [[ -d "$HOME/.acme.sh" && -z $(find "$HOME/.acme.sh/acme.sh ") ]]; then
		...
		if [[ ! -d "$HOME/.acme.sh" ]] || [[ -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
			...
			echoContent red "2.acme.sh script has a bug , you can check [https://github.com/acmesh-official/acme.sh] issues"
			exit 0
		fi
	fi
}
...
checkIP() {
	...
	if [[ -z ${localIP} ]] || ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q '\.' && ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q ':'; then
		...
		read -r -p " Do you want to modify the firewall rules to open ports 443 and 80 through script? [y/n]:" allPortFirewallStatus
		...
		if echo "${localIP}" | awk -F "[,]" '{print $2}' | grep -q "." || echo "${localIP}" | awk -F "[,]" '{ print $2}' | grep -q ":"; then
			echoContent red "\n ---> Multiple ip detected , please confirm whether to close cloudflare 's cloud "
...
installCronTLS() {
	echoContent skyBlue "\nProgress $ 1/${totalProgress} : Add timing maintenance certificate "
	...
	if [[ -z "${coreInstallType}" ]]; then
		...
		if [[ -n "$1" ]]; then
			...
			if [[ "${rollbackV2RayStatus}" == "y" ]]; then
				...
				echoContent green "---> give up the fallback version "
			fi
...
EOF
		...
		echoContent green "---> Configure V2Ray to start up successfully "
	fi
}
...
EOF
		...
		echoContent green "---> Configure Xray to start up successfully "
	fi
}
...
updateNginxBlog() {
	...
	if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
		...
		echoContent green "--->The replacement of the pseudo-station succeeded "
	else
...
unInstall() {
	...
	if [[ "${coreInstallType}" == "1" ]]; then
		handleXray stop
		rm -rf /etc/systemd/system/xray.service
		echoContent green "---> Delete Xray boot auto-start completed "

	elif [[ "${coreInstallType}" == "2" ]]; then

		handleV2Ray stop
		rm -rf /etc/systemd/system/v2ray.service
		echoContent green "---> Delete V2Ray boot auto-start completed "
...
warpRouting() {
	...
	#echoContent yellow "4. Cannot be used after restarting, you can also uninstall and reinstall "
	#install warp _
	if [[ -z $(which warp-cli) ]]; then
		echo
		read -r -p "WARP is not installed, whether to install ? [y/n]:" installCloudflareWarpStatus
...
customV2RayInstall() {
	...
	echoContent yellow "VLESS is pre-installed, 0 is installed by default , if only 0 needs to be installed , just select 0 "
	...
	if [[ "${selectCustomInstallType}" =~ ^[0-5]+$ ]]; then
		...
		echoContent red "--->The input is invalid "
		customV2RayInstall
	fi
}
...
menu() {
	...
	echoContent green " Current version : v2.5.70"
	...
	echoContent yellow "17.Install BBR and DD scripts "
